...
import (
	...
	"github.com/TCP404/OneTiny-base/core/util"
...
func showDir(c *gin.Context, rel string) {
	files := readDir(c, filepath.Join(args.RootPath, rel))
	html := util.GenerateIndexHTML(files, rel, args.IsAllowUpload)
...
func download(c *gin.Context, rel string) {
	...
	fileAbsPath := filepath.Join(args.RootPath, rel)
...
func readDir(c *gin.Context, absPath string) []model.FileStruction {
	...
	relPath := strings.TrimPrefix(absPath, args.RootPath)
	...
	for i, f := range dirEntries {
		...
		if fType == fs.ModeDir { // 将目录的 size 设置为 0，文件则照常
			size = 0
			deviceAbsPath += "/" // 如果是目录，不在路径末尾加上分隔符的话，返回上一级会有问题
			urlRelPath += "/"
		}
